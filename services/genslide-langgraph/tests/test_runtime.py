...
from genslide_langgraph.api import create_app
from genslide_langgraph.bff import Claim, CommitOutcome, SettleOutcome
from genslide_langgraph.config import Settings
from genslide_langgraph.domain import ExecuteRequest, Memory, ServiceError, WorkResult
...
async def test_pod_capacity_is_rejected_before_second_claim() -> None:
    ...
    runtime = ExecutionRuntime(bff, engine, settings(planning_concurrency=1))
...
class FakeEngine:
    ...
    async def run(self, key: str, req: ExecuteRequest, memory: Memory, materials: str) -> WorkResult:
        ...
        requirements.update(req.requirement_updates)
        return WorkResult(
...
class FakeBFF:
    ...
    def _session(self, req: ExecuteRequest) -> dict:
        return self.sessions.setdefault(
            req.session_key(),
            {"version": 0, "expires": datetime.now(timezone.utc) + timedelta(hours=4)},
        )
    ...
    async def commit_result(self, req: ExecuteRequest, claim: Claim, payload) -> CommitOutcome:
        ...
        if action["status"] != "committed":
            ...
            action["receipt"] = f"receipt-{req.action_id}"
        ...
        return CommitOutcome(
            ...
            session_version=action["version"],
            session_expires_at=session["expires"],
...
async def test_claim_version_commit_and_next_turn() -> None:
    ...
    first = await runtime.prepare(request("action-one"))
    assert first.claim.session_version == 0
    assert first.claim.runtime_initialized is False
    result = await runtime.perform(first)
    assert result["status"] == "completed"
    assert bff.sessions[first.key]["version"] == 1
    assert runtime._metadata[first.key].version == 1

    second = await runtime.prepare(request("action-two", version=1))
    assert second.claim.runtime_initialized is True
...
async def test_commit_ack_loss_reconciles_without_rerunning() -> None:
    ...
    prepared = await runtime.prepare(request("lost-ack"))
    result = await runtime.perform(prepared)
    assert result["status"] == "completed"
    assert result["receipt"] == "receipt-lost-ack"
    assert bff.settle_reasons == ["result_ack_lost"]
    assert bff.commit_count == 1
    assert engine.runs == 1
    assert runtime._metadata[prepared.key].version == 1
    await runtime.aclose()


@pytest.mark.asyncio
async def test_cancel_while_claim_waits_settles_unknown_claim() -> None:
    bff, engine = FakeBFF(), FakeEngine()
...
@pytest.mark.asyncio
async def test_disconnect_cancels_uncommitted_work_and_settles_reason() -> None:
    bff, engine = FakeBFF(), FakeEngine()
    engine.run_gate = asyncio.Event()
    runtime = ExecutionRuntime(bff, engine, settings())
    prepared = await runtime.prepare(request("disconnect-before-commit"))
    disconnected = asyncio.Event()

    async def is_disconnected() -> bool:
        return disconnected.is_set()

    task = asyncio.create_task(runtime.perform(prepared, is_disconnected=is_disconnected))
    ...
    assert bff.actions["disconnect-before-commit"]["status"] == "closed"
    assert engine.deletes == [prepared.key]
    assert not runtime._busy
    await runtime.aclose()


@pytest.mark.asyncio
async def test_cancelled_commit_ack_preserves_authoritative_success() -> None:
    bff, engine = FakeBFF(), FakeEngine()
    bff.commit_ack_gate = asyncio.Event()
    runtime = ExecutionRuntime(bff, engine, settings())
    prepared = await runtime.prepare(request("commit-disconnect-race"))
    disconnected = asyncio.Event()
    ...
    assert result["status"] == "completed"
    assert result["receipt"] == "receipt-commit-disconnect-race"
    assert prepared.committed is True
    assert prepared.committed_response is result
    assert bff.settle_reasons == ["result_ack_lost"]
    assert bff.actions["commit-disconnect-race"]["status"] == "committed"
    assert engine.runs == bff.commit_count == 1
    await runtime.aclose()


@pytest.mark.asyncio
async def test_api_auth_validation_json_and_sse_contract() -> None:
    bff, engine = FakeBFF(), FakeEngine()
    ...
    async with app.router.lifespan_context(app):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            body = request("api-json").model_dump(mode="json")
            path = "/v1/actions/api-json/execute"
            unauthorized = await client.post(path, json=body)
            assert unauthorized.status_code == 401
            assert unauthorized.json()["code"] == "SERVICE_AUTH_REQUIRED"
            ...
            stream_request = request("api-sse", version=1).model_dump(mode="json")
            stream = await client.post(
                "/v1/actions/api-sse/execute",
                json=stream_request,
            ...
            oversized = await client.post(
                "/v1/actions/api-json/execute",
                content=b"{" + b" " * 600,
                headers=headers | {"Content-Type": "application/json"},
            )
            assert oversized.status_code == 413

            invalid = request("bad-api").model_dump(mode="json")
            invalid["engine"] = "not-an-engine"
            invalid["authorization"] = "sensitive-bff-grant"
            validation = await client.post("/v1/actions/bad-api/execute", json=invalid, headers=headers)
            assert validation.status_code == 422
            assert "sensitive-bff-grant" not in validation.text


@pytest.mark.asyncio
async def test_asgi_disconnect_cancels_sse_runtime() -> None:
    bff, engine = FakeBFF(), FakeEngine()
    engine.run_gate = asyncio.Event()
    app = create_app(settings=settings(), bff=bff, engine=engine)
    request_body = request("asgi-disconnect").model_dump(mode="json")
    ...
    async def send(message):
        ...
        if message.get("type") == "http.response.body" and b"event: accepted" in message.get("body", b""):
            async def disconnect_after_engine_starts():
                await asyncio.wait_for(engine.started.wait(), timeout=1)
                disconnect.set()

            asyncio.create_task(disconnect_after_engine_starts())
    ...
    scope = {
        ...
        "raw_path": b"/v1/actions/asgi-disconnect/execute",
        "query_string": b"",
        "root_path": "",
        "headers": headers,
        "client": ("test", 1234),
    ...
    async with app.router.lifespan_context(app):
        ...
        assert any(message.get("status") == 200 for message in sent if message["type"] == "http.response.start")
        assert bff.settle_reasons == ["client_disconnected"]

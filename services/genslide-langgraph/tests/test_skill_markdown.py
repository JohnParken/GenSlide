...
def test_markdown_metadata_body_hash_and_default_target(tmp_path):
    path, raw = write_md(tmp_path, metadata="metadata:\n  version: '2'\n")
...
def test_duplicate_ids_symlink_directories_and_unrelated_files_fail(tmp_path):
    ...
    registry = SkillRegistry(tmp_path)
    assert set(registry.skills) == {"same"}
...
def test_removed_or_changed_markdown_requires_reload(tmp_path):
    ...
    changed = SkillRegistry(tmp_path).get("writing", "markdown-skill")
    assert changed["hash"] != registry.get("writing", "markdown-skill")["hash"]
...
async def test_workflow_receives_body_and_binds_markdown_skill(tmp_path):
    write_md(tmp_path)
    registry = SkillRegistry(tmp_path)
    values = dict(engine="langgraph", tenant_id="t", user_id="u", session_id="s", runtime_epoch="e",
                  action_id="a", authorization="x", expected_session_version=0, target_kind="writing")
    ...
    model = Model()
    created = await execute(ExecuteRequest(**values, operation="create_outline", skill_id="markdown-skill"),

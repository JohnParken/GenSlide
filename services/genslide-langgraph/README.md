# GenSlide LangGraph 一期 API

独立安装、独立部署，不 import 根目录旧工程或另一服务包。
框架版本：`langgraph==0.6.11`；推荐 Python 3.12（支持 3.11–3.12）。
...
```sh
uv run --locked uvicorn genslide_langgraph.api:create_app --factory \
...
执行请求至少包含：
`api_contract_version="1"`、`engine="langgraph"`、
...
```sh
uv run --locked uvicorn genslide_langgraph.mock_bff:create_mock_bff --factory --host 127.0.0.1 --port 8001
```
...
`genslide-langgraph-0.genslide-langgraph.<namespace>.svc:8000`），不能轮询 headless 地址。

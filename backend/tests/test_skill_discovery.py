...
def put(root, skill_id, value):
    ...
    metadata += f"  supported_outputs: {value.get('supported_outputs', 'text,document,presentation')}\n"
    metadata += f"  default_output: {value.get('default_output', 'text')}\n"
    metadata += f"  priority: '{value.get('priority', '0')}'\n"
...
def test_selection_errors_and_defensive_copy(tmp_path):
    put(tmp_path, "custom", skill("custom", supported_outputs="text"))
    registry = SkillRegistry(tmp_path)
...
def test_supported_outputs_default_output_and_priority_are_discovered(tmp_path):
    put(tmp_path, "focused", skill("focused", supported_outputs="document,text", default_output="document", priority="7"))
    entry = SkillRegistry(tmp_path).list_skills()[0]
    assert entry["supported_outputs"] == ["document", "text"]
    assert entry["default_output"] == "document"
    assert entry["priority"] == 7

def test_reload_reflects_metadata_changes(tmp_path):
    put(tmp_path, "reloadable", skill("reloadable", priority="1"))
    registry = SkillRegistry(tmp_path)
    assert registry.get("writing", "reloadable")["priority"] == 1
    path = tmp_path / "reloadable" / "SKILL.md"
    path.write_text(path.read_text(encoding="utf-8").replace("priority: '1'", "priority: '9'"), encoding="utf-8")
    registry.reload()
    assert registry.get("writing", "reloadable")["priority"] == 9
...
async def test_workflow_inherits_bound_skill_and_rejects_other_skill(tmp_path):
    ...
    registry = SkillRegistry(tmp_path)
    values = dict(engine="agentscope", tenant_id="t", user_id="u", session_id="s",

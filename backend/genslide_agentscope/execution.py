...
class ExecutionContext:
    """Immutable action scope; never a user-owned persistent workspace.

    No credentials or source text are included, so this descriptor is safe to inspect.
    The selected Skill is separately pinned by a registry snapshot before model work.
    """
    tenant_id: str
    user_id: str
    session_id: str
...
class ExecutionRuntime:
    ...
    async def prepare(self, request: ExecuteRequest) -> PreparedExecution:
        ...
        # A unified turn may choose a deliverable after the model starts; reserve the
        # stricter generation slot before claim so effect selection cannot bypass limits.
        bucket = "generation"
        ...
        try:
            ...
            now = _utcnow()
            operation_limit = self.settings.generation_timeout_seconds
            ...
            return PreparedExecution(
                ...
                context=ExecutionContext(
                    request.tenant_id, request.user_id, request.session_id, request.action_id,
    ...
    async def perform(
        ...
        *,
        ...
        try:
            try:
                self._workspaces.sweep()
                workspace = self._workspaces.create(
                    execution.request.action_id, execution.execution_instance_id
                )
            ...
            work_task = asyncio.create_task(self._pipeline(execution, workspace, emit))
    ...
    async def _pipeline(
        ...
        for file_id in request.current_file_ids:
            ...
            async with self._transfer_slots:
                downloaded = await self.bff.download_file(
                    request, execution.claim, file_id, workspace.inputs, self.settings.max_download_bytes
                )
            path = self._validate_download(downloaded, workspace.inputs)
        ...
        effect = work.effect
        kind = self._effect_kind(execution, work)
        if effect == "deliverable" and kind in {"document", "presentation"}:
            await self._emit_progress(execution, emit, "rendering_artifact")
            assert work.content is not None
            rendered_path = await self._render(execution, work.content, workspace, kind)
            self._check_workspace(workspace)
            async with self._transfer_slots:
    ...
    def _validate_work(self, execution: PreparedExecution, work: WorkResult) -> None:
        if len(work.memory.model_dump_json(exclude={"content"}).encode("utf-8")) > self.settings.max_memory_bytes:
            raise ServiceError("CONTEXT_CAPACITY", 413)
        if "memory" in work.result:
            raise ServiceError("ENGINE_CONTRACT_ERROR", 502)
        if work.effect == "deliverable":
            if work.content is None and work.deliverable is None:
                raise ServiceError("GENERATED_CONTENT_MISSING", 502)
            if work.content is None:
                work.content = work.deliverable
        elif work.content is not None or work.deliverable is not None:
            raise ServiceError("ENGINE_CONTRACT_ERROR", 502)
        try:
    ...
    def _snapshot_for_commit(self, work: WorkResult) -> dict[str, Any]:
        snapshot = snapshot_from_memory(work.memory)
        try:
    ...
    @staticmethod
    def _effect_kind(execution: PreparedExecution, work: WorkResult) -> str:
        output = work.output or work.result.get("output", {})
        kind = output.get("target_kind") if isinstance(output, Mapping) else None
        if kind not in {"writing", "document", "presentation"}:
            kind = execution.request.target_kind
        return kind

    @staticmethod
    def _check_workspace(workspace: Workspace) -> None:
        try:
            usage = workspace.check_usage()
        except ServiceError:
            raise
        except BaseException as exc:
            raise ServiceError("WORKSPACE_UNAVAILABLE", 503) from exc
        if not hasattr(usage, "used_bytes") or not hasattr(usage, "free_bytes"):
            raise ServiceError("WORKSPACE_CONTRACT_ERROR", 500)

    async def _render(self, execution: PreparedExecution, content: Content,
                      workspace: Workspace, kind: str) -> Path:
        output = await run_cpu_stage(
            "render_content",
            {"kind": kind, "content": content.model_dump(), "directory": str(workspace.outputs)},
            slots=self._cpu_slots,
            timeout=self._remaining(execution),
        )
        path = Path(output).resolve()
        try:
            path.relative_to(workspace.outputs.resolve())
        except ValueError as exc:
            raise ServiceError("ARTIFACT_INVALID", 500) from exc
        suffix = ".docx" if kind == "document" else ".pptx"
    ...
    async def _commit(
        ...
        snapshot = self._snapshot_for_commit(work)
        payload = {
            ...
            "mode": request.mode,
            "target_kind": self._effect_kind(execution, work),
            "effect": work.effect,
            "result": work.result,
        ...
        response = {
            ...
            "lifecycle_version": outcome.lifecycle_version,
            "session_expires_at": outcome.session_expires_at,
            "receipt": outcome.receipt,
            "effect": work.effect,
            "reply": work.reply,
            "outline": work.outline.model_dump() if work.outline is not None else None,
            "deliverable": work.deliverable.model_dump() if work.deliverable is not None else None,
            "output": work.output,
            "result": work.result,
            "content": work.content.model_dump() if work.content is not None else None,
            "files": [reference.as_dict() for reference in files],
            "snapshot": outcome.snapshot or snapshot,
    ...
    def _validate_commit(self, execution: PreparedExecution, outcome: CommitOutcome) -> CommitOutcome:
        ...
        if (
            ...
            or outcome.lifecycle_version != execution.claim.lifecycle_version
    ...
    def _commit_from_settlement(self, execution: PreparedExecution, settled: SettleOutcome) -> CommitOutcome:
        if (
            ...
            or settled.lifecycle_version != execution.claim.lifecycle_version
        ...
        return CommitOutcome(
            ...
            lifecycle_version=settled.lifecycle_version or execution.claim.lifecycle_version,
    ...
    async def _read_committed_memory(self, key: str, request: ExecuteRequest, claim: Claim) -> Memory:
        if claim.session_expires_at <= _utcnow():
        ...
        if claim.snapshot is None:
            ...
            return Memory()
        if claim.snapshot.get("schema_version") != 1:
            raise ServiceError("SNAPSHOT_VERSION_UNSUPPORTED", 409)
        try:
            snapshot_size = len(json.dumps(claim.snapshot, ensure_ascii=False, allow_nan=False).encode("utf-8"))
            if snapshot_size > self.settings.max_snapshot_bytes:
                raise ServiceError("CONTEXT_CAPACITY", 413)
            checked = memory_from_snapshot(claim.snapshot)
        except ServiceError:
            raise
        except (ValueError, TypeError) as exc:
            raise ServiceError("CONTEXT_INVALID", 409) from exc
        if len(checked.model_dump_json(exclude={"content"}).encode("utf-8")) > self.settings.max_memory_bytes:
            raise ServiceError("CONTEXT_CAPACITY", 413)
        return checked
    ...
    def _validate_claim(self, request: ExecuteRequest, instance_id: str, claim: Claim) -> None:
        if (
            ...
            or claim.lifecycle_version != request.expected_lifecycle_version
        ):

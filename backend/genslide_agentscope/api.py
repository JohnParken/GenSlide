...
class RequestGuardMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if (execute_route or path in {"/internal/metrics", "/v1/skills/reload"}) and self.auth_enabled:
            authorization = next(
...
def create_app(
    ...
    async def lifespan(app: FastAPI):
        ...
        async def sweep_workspaces():
            while True:
                try:
                    await asyncio.to_thread(runtime._workspaces.sweep)
                except Exception:
                    logging.getLogger(__name__).exception("Workspace sweep failed")
                await asyncio.sleep(min(60.0, settings.workspace_stale_seconds))

        sweeper = asyncio.create_task(sweep_workspaces())
        try:
            yield
        finally:
            sweeper.cancel()
